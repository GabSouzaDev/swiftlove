...
export default function SecretLetterPuzzle({ fromName, toName, onSolved }: SecretLetterPuzzleProps) {
  const [currentStep, setCurrentStep] = useState(1);
  const [userAnswer, setUserAnswer] = useState("");
  const [currentRiddle] = useState(riddles[Math.floor(Math.random() * riddles.length)]);
  const [heartSequence] = useState(heartSequences[Math.floor(Math.random() * heartSequences.length)]);
  ...
  const checkAnswer = () => {
    ...
    if (normalizedAnswer === normalizedCorrect) {
      ...
      toast({
        ...
        description: attempts >= 1 ? "Quer uma dica? Clique no botão de dica!" : "Tente novamente!",
        variant: "destructive"
      });
    }
  };
  ...
  const handleHeartClick = (heartNumber: number) => {
    ...
    const isCorrectSoFar = newClickedHearts.every((heart, index) => heart === expectedSequence[index]);
    ...
    if (newClickedHearts.length === heartSequence.length) {
      setTimeout(() => {
        ...
        toast({
          ...
          description: "Você desbloqueou a carta secreta!",
        });
              ...
              <CardContent className="space-y-6">
                <div className="text-center">
                  <p className="text-lg text-gray-700 mb-6 font-dancing">
                    Clique nos corações na sequência: {heartSequence.join(" → ")}
                  </p>
            ...
            <Card className="love-card border-2 border-green-500 bg-gradient-to-br from-green-50 to-green-100">
              <CardContent className="text-center py-8">
                ...
                <p className="text-green-600 font-dancing text-lg">
                  Você desbloqueou a carta secreta!
                </p>
